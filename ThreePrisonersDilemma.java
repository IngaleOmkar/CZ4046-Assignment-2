public class ThreePrisonersDilemma {
	...
	class ReversibleTriggerPlayer extends Player {
		// Cooperates until the player is berayed. 
	...
	class LimitedReversibleTriggerPlayer extends Player {
		...
		int selectAction(int n, int[] myHistory, int[] oppHistory1, int[] oppHistory2) {
			if(n == 0) return 0; // cooperate on first round 
			if(betrayalToleranceBreached(n, oppHistory1, oppHistory2)) {
				betrayed = true;
				return 1;
			}
			if(!betrayed){
		...
		boolean betrayalToleranceBreached(int n, int[] oppHistory1, int[] oppHistory2) {
			int betrayals = 0;
			for(int i = 0 ; i < oppHistory1.length ; i++) {
				if(oppHistory1[i] == 1 || oppHistory2[i] == 1) betrayals++;
			}
...
//	This is a helper function needed by scoresOfMatch.
	...
	Player makePlayer(int which) {
		...
		case 6: return new SuspiciousT4TPlayer();
		case 7: return new AlternatePlayer();
		case 8: return new LimitedForgivenessPlayer();
		case 9: return new GrimTriggerPlayer();
		case 10: return new WinStayLoseShiftPlayer();
		case 11: return new IncreasingGreedPlayer();
		case 12: return new ReversibleTriggerPlayer();
		case 13: return new LimitedReversibleTriggerPlayer();
		case 14: return new CooperativeT4TPlayer();
		}
